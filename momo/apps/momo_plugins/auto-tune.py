...
class auto_tune(plugins.Plugin):
    ...
    def _ensure_presets_dir(self):
        ...
        try:
            ...
            logging.exception(f"Failed to create presets directory {self.presets_dir}: {e!s}")
            raise e
    ...
    def _get_preset_files(self):
        ...
        try:
            ...
            logging.exception(f"Error reading presets directory: {e!s}")
            return []
    ...
    def _save_preset(self, preset_name):
        ...
        try:
            ...
            logging.exception(f"Error saving preset '{preset_name}': {e!s}")
            raise e
    ...
    def _load_preset(self, preset_name):
        ...
        try:
            ...
            logging.exception(f"Error loading preset '{preset_name}': {e!s}")
            return False, f"Error loading preset: {e!s}"
    ...
    def _delete_preset(self, preset_name):
        ...
        try:
            ...
            logging.exception(f"Error deleting preset '{preset_name}': {e!s}")
            return False
    ...
    def showChistos(self, stats=None, sort_key="_all_actions"):  # stats is list of specific to show, else all
        ...
        try:
            ...
            for (ch, _count) in channel_order:
                if ch == -1:
                    ret += "<th>All</th>"
                else:
                    ret += "<th>%d</th>" % ch
            ...
            for s in stats:
                ...
                if s not in self._chistos:
                    ...
                    for (ch, _dummy) in channel_order:
                        if ch in chisto:
    ...
    def showEditForm(self, request):
        path = request.path if request.path.endswith("/update") else f"{request.path}/update"
            ...
            for p in sorted(sec):
                if type(sec[p]) in [int, str, float, bool]:
                    ...
                    if cls == "bool":
                        ...
                        ret += '<input type=radio id="{}" name="{}" value="{}" {}>&nbsp;True<br>'.format(
                        iname, iname, "True", checked)
                        checked = " checked" if not sec[p] else ""
                        ret += '<input type=radio id="{}" name="{}" value="{}" {}>&nbsp;False'.format(
                        iname, iname, "False", checked)
                        ret += "</td>"
                    else:
                        ret += f"<th>{p}</th>"
                        ret += f'<td><input type=text id="{iname}" name="{iname}" size="5" value="{sec[p]}"></td>'
    ...
    def showInteractions(self):
        ...
        if self._agent:
            ...
            for (_id, ap) in sorted(self._known_aps.items(), key=lambda x: x[1]["AT_lastseen"], reverse=True):
                lmac = ap["mac"].lower()
                if ap["hostname"] == "<hidden>" and not self.options["show_hidden"]:
                    logging.debug("Skipping {} '{}'".format(ap["hostname"], lmac))
                    numHidden += 1
                    continue  # skip hidden APs
                if ap["hostname"] == "" and not self.options["show_hidden"]:
                    logging.debug("Skipping no-name {} '{}'".format(ap["hostname"], lmac))
                    numHidden += 1
                    continue  # skip hidden APs
                if ap["hostname"] is None and not self.options["show_hidden"]:
                    logging.debug("Skipping None {} '{}'".format(ap["hostname"], lmac))
                    numHidden += 1
                    continue  # skip hidden APs
                numVisible += 1
                logging.debug("Not skipping '{}'".format(ap["hostname"]))
                if ap["AT_visible"]:
                    ret += "<tr><td>{}</td>".format(html.escape(ap["hostname"]))
                else:
                    ret += "<tr><td><i>{}</i></td>".format(html.escape(
                        ap["hostname"]))  # italicise hosts not currently visible
                ret += "<td>{}</td><td>{}</td>".format(ap["mac"], ap["channel"])
                ret += "<td>%d</td>" % int(now - ap["AT_lastseen"])  # time since last interaction
                ret += "<td>{}</td>".format(ap["rssi"])
        ...
        try:
            if request.method == "GET":
                if path == "/" or not path:
                    ...
                    if self.options.get("show_interactions"):
        ...
        try:
            ...
            if len(self._unscanned_channels) == 0:
                if "restrict_channels" in self.options:
                    ...
                    logging.info(f"Repopulating from allowed list: {agent._allowed_channels}")
    ...
    def on_bcap_wifi_ap_lost(self, agent, event):
        try:
            ...
            if apID not in self._known_aps:
                ...
                logging.warning(f"Unknown AP '{apID}' seen leaving")
            elif not self._known_aps[apID]["AT_visible"]:
