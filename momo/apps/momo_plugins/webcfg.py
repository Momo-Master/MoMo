...
_limiter = _RateLimiter(capacity=0, per_seconds=0)  # Disabled
...
def get_app(allow_unauth: bool = False) -> Flask:
    app = Flask(__name__)
    ...
    def _limit() -> Response | None:  # type: ignore[override]
        ...
        ok, code = _auth_ok(allow_unauth)
        if not ok:
            _REQ_TOTAL[str(code)] += 1
    ...
    def index() -> Response:
        global _REQ_TOTAL
        _REQ_TOTAL["200"] += 1
        return jsonify({"status": "ok", "service": "webcfg"})
...
def init(cfg: dict) -> None:
    ...
    _ACTIVE = True
    _ACTIVE_GAUGE = 1
    logging.info("[webcfg] started on 127.0.0.1:%d", port)
...
def shutdown() -> None:
    ...
    _ACTIVE = False
    _ACTIVE_GAUGE = 0
    logging.info("[webcfg] stopped")


def get_metrics() -> dict:
    return {
        "momo_webcfg_requests_total": dict(_REQ_TOTAL),
        "momo_webcfg_active": _ACTIVE_GAUGE,
    }
