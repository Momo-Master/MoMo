...
class Webgpsmap(plugins.Plugin):
    ...
    def on_webhook(self, path, request):
        ...
        if not self.ready:
            try:
                ...
                logging.exception(f"[webgpsmap] on_webhook NOT_READY error: {error}")
                return None
            ...
            if path == "/" or not path:
                ...
                try:
                    ...
                    logging.exception(f"[webgpsmap] on_webhook / error: {error}")
                    return None
                response_status = 200
                response_mimetype = "application/xhtml+xml"
                response_header_contenttype = "text/html"
                ...
                try:
                    ...
                    logging.exception(f"[webgpsmap] on_webhook all error: {error}")
                    return None
                ...
                try:
                    ...
                    logging.exception(f"[webgpsmap] on_webhook offlinemap: error: {error}")
                    return None
        ...
        try:
            ...
            logging.exception(f"[webgpsmap] on_webhook CREATING_RESPONSE error: {error}")
            return None
    ...
    def load_gps_from_dir(self, gpsdir, newest_only=False):
        ...
        for pos_file in all_geo_or_gps_files:
            try:
                ...
                self.SKIP += pos_file
                logging.exception(f"[webgpsmap] JSONDecodeError in: {pos_file} - error: {error}")
                continue
            except ValueError as error:
                self.SKIP += pos_file
                logging.exception(f"[webgpsmap] ValueError: {pos_file} - error: {error}")
                continue
            except OSError as error:
                self.SKIP += pos_file
                logging.exception(f"[webgpsmap] OSError: {pos_file} - error: {error}")
                continue
    ...
    def get_html(self):
        ...
        try:
            ...
            html_data = open(template_file).read()
        except Exception as error:
            logging.exception(f"[webgpsmap] error loading template file {template_file} - error: {error}")
        return html_data
    ...
    def password(self):
        ...
        if os.path.isfile(password_file_path):
            try:
                ...
                logging.exception(f"[webgpsmap] OS error loading password: {password_file_path} - error: {format(error)}")
            except:
                logging.exception(f"[webgpsmap] Unexpected error loading password: {password_file_path} - error: {sys.exc_info()[0]}")
                raise
        return return_pass
