...
class enable_deauth(plugins.Plugin):
    ...
    def on_touch_ready(self, touchscreen):
        ...
        self.hasTouch = self._touchscreen and self._touchscreen.running
        logging.info(f"[DEAUTH] Touchscreen {touchscreen!r}")

    # click on release
    def on_touch_release(self, ts, ui, ui_element, touch_data):
        logging.debug(f"[DEAUTH] Touch press: {touch_data!r}")
        try:
            if ui_element == "deauth_count":
                logging.debug("Toggling {}".format(repr(self._agent._config["personality"]["deauth"])))
                self._agent._config["personality"]["deauth"] = self._ui._state._state["deauth_count"].state
                logging.info("Toggled deauth to {}".format(repr(self._ui._state._state["deauth_count"].state)))
    ...
    def on_ui_setup(self, ui):
        ...
        try:
            ...
            logging.info(f"enable deauth ui error: {err!r}")
