...
class UploadConvertPlugin(Plugin):
    ...
    def _convert_and_upload(self):
        # Convert all .pcap files to .hc22000 - NO WHITELIST FILTERING
        pcap_files = [f for f in os.listdir(self.handshake_dir)
                      if f.endswith(".pcap")]  # Process ALL files
        if pcap_files:
            for pcap_file in pcap_files:
                subprocess.run(["hcxpcapngtool", "-o", self.combined_file, os.path.join(self.handshake_dir, pcap_file)], check=False)
