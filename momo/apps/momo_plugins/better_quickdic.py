...
class QuickDic(plugins.Plugin):
    ...
    def on_loaded(self):
        ...
        check = subprocess.run(
            ("/usr/bin/dpkg -l aircrack-ng | grep aircrack-ng | awk '{print $2, $3}'"), shell=True, stdout=subprocess.PIPE, check=False)
        check = check.stdout.decode("utf-8").strip()
        if check != "aircrack-ng <none>":
            logging.info(f"[quickdic] Found {check}")
        else:
    ...
    def on_handshake(self, agent, filename, access_point, client_station):
        ...
        if not result:
            ...
            if result2 != "KEY NOT FOUND":
                ...
                if self.options["id"] is not None and self.options["api"] is not None:
